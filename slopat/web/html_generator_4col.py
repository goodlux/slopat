"""Beautiful HTML generation for slop.at with 4-column layout"""
...
class HTMLGenerator:
    """
    Generate beautiful HTML pages with 4-column layout:
    concepts | related | content | outline
    """
    ...
    def generate_slop_page(
        ...
        """Generate a complete slop.at page with 4-column layout"""
        ...
        # Process content and extract outline
    ...
    def _process_content_by_type(self, content: str, doc_type: DocumentType) -> Tuple[str, List[DocumentOutline]]:
        ...
        if doc_type == DocumentType.CONVERSATION:
            ...
            # Plain text - wrap in paragraphs, no outline
            paragraphs = content.split('\n\n')
            formatted = '\n'.join(f'<p>{paragraph.replace(chr(10), "<br>")}</p>' 
                                 for paragraph in paragraphs if paragraph.strip())
            return formatted, []
    
    def _format_conversation_content(self, content: str) -> Tuple[str, List[DocumentOutline]]:
        ...
        for i, line in enumerate(lines):
            ...
            if speaker_match:
                ...
                # Add to outline if new speaker or significant gap
                if speaker != current_speaker:
                    outline_id = f"speaker-{len(outline)}"
                    outline.append(DocumentOutline(1, f"{speaker}", outline_id, i))
                    current_speaker = speaker
                
                formatted_lines.append(f'''
                <div class="conversation-line" id="line-{i}">
        ...
        return '\n'.join(formatted_lines), outline
    
    def _format_markdown_content(self, content: str) -> Tuple[str, List[DocumentOutline]]:
        """Format markdown content and extract heading outline"""
        lines = content.split('\n')
        formatted_lines = []
        outline = []
        
        for i, line in enumerate(lines):
            # Check for headers and add to outline
            ...
            if header_match:
                ...
                outline.append(DocumentOutline(level, text, outline_id, i))
                
                # Create header with ID for linking
                formatted_lines.append(f'<h{level} id="{outline_id}">{html.escape(text)}</h{level}>')
            else:
    ...
    def _generate_complete_html(
        ...
        """Generate the complete HTML page with 4-column layout"""
...
<head>
    ...
    <style>
        ...
        /* Layout: concepts | related | content | outline */
        body {{
            margin: 0;
        ...
        .container {{
            display: grid;
            grid-template-columns: 250px 300px 1fr 250px;
            gap: 1.5rem;
            max-width: 1600px;
            margin: 0 auto;
            padding: 2rem 1rem;
        ...
        @media (max-width: 1400px) {{
            .container {{
                grid-template-columns: 220px 280px 1fr 220px;
                gap: 1rem;
            }}
        }}
...
<body>
    <div class="container">
        ...
        <aside class="concepts-sidebar">
            {concepts_sidebar_html}
        </aside>
        
        <aside class="related-content">
            <h3>🔗 Related Slops</h3>
            <div id="related-content">
                <p style="color: #6b7280; font-style: italic; font-size: 0.8rem;">
        ...
        function updateRelatedContent(conceptText, domain) {{
            ...
            relatedDiv.innerHTML = `
                <div style="margin-bottom: 1rem; font-size: 0.85rem;">
                    <strong>${{conceptText}}</strong><br>
                    <span style="color: #6b7280; font-size: 0.75rem;">${{domain}} concept</span>
                </div>
                <div style="color: #6b7280; font-style: italic; font-size: 0.75rem;">
                    Related slops would appear here in a full implementation
                </div>
            `;
        }}
    ...
    def _generate_concepts_sidebar_html(self, sidebar_data: Dict) -> str:
        ...
        for domain, concepts in sidebar_data["concepts_by_domain"]:
            ...
            for concept in sorted_concepts[:8]:  # Show top 8 per domain
                link_id = self._generate_concept_link_id(concept)
                html_parts.append(f"""
